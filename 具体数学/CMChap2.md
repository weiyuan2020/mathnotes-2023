...
$$\begin{aligned}
    ...
    &= 166650\end{aligned}$$ ⼀般形式 ⽐有确定界限形式 更好处理

$k\rightarrow k+1$\
⼀般形式, $$\sum_{1\leqslant k\leqslant n}a_k =
    \sum_{1\leqslant k+1\leqslant n}a_{k+1} = 
    \sum_{0\leqslant k\leqslant n-1}a_{k+1}$$ 有确定界限形式
$$\sum_{k=1}^{n}a_k = \sum_{k=0}^{n-1}a_{k-1}$$ 不要轻易省略 0项
计算有效性不等同于理解的有效性 $$\sum_{k=0}^{n}k(k-1)(n-k)\quad
    \sum_{k=2}^{n-1}k(k-1)(n-k)$$ 保持求和界限简单大有裨益
...
和式 $$S_n = \sum_{k=0}^{n}a_k$$ 等价于递归式

封闭形式解递归式的值 $\Rightarrow$ 封闭形式解和式 $$\left\{
        \begin{array}{l}
            S_0 = a_0 \\
...
$$
    R_n = A(n)\alpha + B(n)\beta + C(n)\gamma$$ repertoire method
令$R_n=1$ , 则$\alpha=1, \beta=0, \gamma=0$ , $$A(n)=1$$ 令$R_n=n$ ,
则$\alpha=0, \beta=1, \gamma=0$ , $$B(n)=n$$ 令$R_n=n^2$ ,
则$\alpha=0, \beta=-1, \gamma=2$ , $$C(n)= \frac{n(n+1)}{2}$$

::: example
**例 1**. $$\sum_{k=0}^{n}(a+b k)$$
:::
...
**解 1**. $$\left\{
        ...
        \right.$$ $$\left\{
            ...
            \end{array}
        \right.$$ $\alpha = \beta =a, \gamma = b$ $$\begin{aligned}
...
对上述递归情况进行推广 $$
    ...
    \end{aligned}
    \right.$$ $$
    R_n = A(n)\alpha + B(n)\beta + C(n)\gamma +D(n) \delta$$ $\delta=0$
时 ([\[RecuHard\]](#RecuHard){reference-type="ref"
reference="RecuHard"})与([\[ReceEasy\]](#ReceEasy){reference-type="ref"
reference="ReceEasy"})一致, 说明 $A(n), B(n), C(n)$ 不变\
$R_n = n^3$ $$\begin{aligned}
    R_n = R_{n-1} &= n^3 - (n-1)^3 \\
    &= 3n^2 - 3n + 1\end{aligned}$$
解得$\alpha=0, \beta=1, \gamma=-3, \delta=3$ $$\begin{aligned}
    n^3 &= B(n)-3C(n)+3D(n)\\
    &= n-3 \frac{n(n+1)}{2}+3D(n)\end{aligned}$$ $$\begin{aligned}
...
fallings        \end{array}
    \right.$$ $$\left\{
        ...
        \end{array}
    \right.$$ 记$S_n = \frac{T_n}{2^n}$

求和因子 $s_n$ $$\begin{aligned}
...
$$C_n = n+1+\frac{2}{n}\sum_{k=0}^{n-1}C_k, \quad n>1.$$ 消去分母中的$n$
$$
    nC_n = n(n+1)+2\sum_{k=0}^{n-1}C_k, \quad n>1.$$
$n\rightarrow (n-1)$ $$
    (n-1)C_{n-1} = (n-1)n+2\sum_{k=0}^{n-2}C_k, \quad n-1>1.$$
$$nC_n - (n-1)C_{n-1} = 2n+2C_{n-1}, \quad n>2.$$ 递归式化为
$$\begin{array}{l}
        C_0 = C_1 = 0;C_2 = 3
...
$\frac{1}{k}$ 处所产生的基音． $$\begin{aligned}
...
$$\sum_{k\in K} a_k = \sum_{P(k)\in K} a_{P(k)}$$ $P(k)$
为这些整数的任意一个排列。
$$f: J\Rightarrow K, \quad j\in J \quad f(j) \in K$$
$$\sum_{j\in J}a_{f(j)} = \sum_{k\in K} a_k \quad \#f^-(k)$$ 式中
...
**解 4**.  1. 先对$j$ 求和 $$\begin{aligned}
    ...
    \end{aligned}$$ 2. 先对$k$ 求和 $$\begin{aligned}
        S_n 
...
3\. 先用$k+j$ 替换$k$ (先换元，再求和) $$\begin{aligned}
        S_n 
...
的值。我们可以利用这种思路求解$\square_n$ 。 $$\begin{aligned}
...
该式近似等于0到$n$ 之间曲线$f(x)=x^2$ 下的面积 $$\begin{aligned}
    S &= \int_{0}^{n}x^2 dx \\
    &=\frac{n^3}{3}\end{aligned}$$ $\square_n$ 近似等于 $\frac{n^3}{3}$
。 近似的误差$E_n = \square_n - \frac{n^3}{3}$

1\. 近似误差项递归式 $$\begin{aligned}
...
\end{align*}$$
  ...
  : 有限微积分和无限微积分中的运算对比
:::
![[具体数学/CMChap2_table.pdf|CMChap2_table]]

[]{#tab:ContiAndDis001 label="tab:ContiAndDis001"}
...
$$\sum_{a}^{b}g(x)\delta x=f(x)|_{a}^b=f(b)-f(a)$$ 设
...
$$\begin{aligned}
    ...
    &= -(f(a)-f(b)) = -\sum_{a}^{b}g(x)\delta x\end{aligned}$$
$$\sum_{a}^{b}+\sum_{b}^{c} = \sum_{a}^{c}$$
应用：计算下降幂和式的简单方法 $$\sum_{0\leqslant k<n}k
  ^{\underline{m}}
 = \frac{k
  ^{\underline{m+1}}
}{m+1}\Big|_0^n = \frac{n
  ^{\underline{m+1}}
}{m+1},\quad \left(m,n\geqslant 0\quad m,n\in\mathbb{N}^+\right)$$ $m=1$
时，$k
  ^{\underline{1}}
=k$ $$\sum_{0\leqslant k<n}k = \frac{n
  ^{\underline{2}}
}{2}=\frac{n(n-1)}{2}$$
...
负指数下降幂 $$\begin{aligned}
 ...
 &= \frac{1}{(x+1)(x+2)} \\
    ...
    x
  ^{\underline{-m}}
 &= \frac{1}{(x+1)(x+2)\dots(x+m)} \\\end{aligned}$$ $$x
  ^{\underline{3}}
\cdot \frac{1}{x-3+1} = x
  ^{\underline{2}}
 \Rightarrow 
  ...
  ^{\underline{0}}
\cdot \frac{1}{x-0+1} = x
  ^{\underline{-1}}$$ 为什么选用$x  ^{\underline{-1}}=\frac{1}{x+1}$ 而不是 $x  ^{\underline{-1}}=\frac{1}{x+1}$ 作为下降阶乘幂的拓展定义?[^4]
...
$m<0$ 时，$$\Delta x
  ^{\underline{m}}
 = m x
  ^{\underline{m-1}}$$ 是否仍成立? $$\begin{aligned}
    \Delta x
  ^{\underline{-2}}
...
[]{#tab:55rightside label="tab:55rightside"}
...
可得$\delta u(x)=1, v(x)=2^x,Ev = 2^{x+1}$ $$\begin{aligned}
        \sum x2^x\delta x
...
的上下标范围存在误解.\
记$\sum_{0\leqslant k\leqslant n} k 2^k = S_n$ $$\begin{aligned}
...
类比$\int x\ln{x} dx$ $$\begin{aligned}
    I &= \int x\ln{x} dx \\
...
$\Delta u(x) = x  ^{\underline{-1}}$ , $v(x) = \frac{1}{2}x  ^{\underline{2}}$ ,$Ev(x) = v(x+1)=\frac{1}{2}(x+1)  ^{\underline{2}}$ $$\begin{aligned}
    \sum x H_x \delta x
    &= \frac{1}{2}x
  ^{\underline{2}}
...
x
 ...
 \delta x\\
  ...
  ^{\underline{2}}
+C\end{aligned}$$ $$\sum_{0\leqslant k< n} k H_k 
    = \sum_{x=0}^{n-1}x H_x \delta x = \frac{(n-1)
  ^{\underline{2}}
}{2}(H_{n-1}-\frac{1}{2})$$ 教材上是 $$\sum_{0\leqslant k< n} k H_k 
    = {\color{red}\sum_{x=0}^{n}x H_x \delta x = \frac{n
  ^{\underline{2}}
}{2}(H_{n}-\frac{1}{2})}$$ 借助有限微积分的原理，我们 很容易地记住
$$\sum_{0\leqslant k<n}k = \frac{n
  ^{\underline{2}}
}{2} = n(n-1)/2$$ [^5] myex $\sum_{0\leqslant k < n}H_k$
$$\begin{array}{ll}
        u(x) = H_x & \Delta v(x) = x
  ^{\underline{0}}
=1 \\
...
均有 $$\sum_{k\in F}a_k \leqslant A$$ 那么我们定义 $\sum_{k\in K} a_k$
...
⾮负和式$\rightarrow$ 和式中有⾮负项与负项 1. $$\begin{aligned}
...
2\. 双向⽆限 $\sum_k a_k$ $$\begin{array}{ll}
...
<div>
$$\sum_{k\in K}a_k = \sum_{k\in K}\tikz[remember picture,baseline=(ak1.base)]
	\node[minimum size=0pt,inner sep=0pt,](ak1){$ a_k^+ $ };
 - \sum_{k\in K}\tikz[remember picture,baseline=(ak2.base)]
	\node[minimum size=0pt,inner sep=0pt,](ak2){$ a_k^- $ };$$
</div>
...
$$\begin{array}{llll}
    ...
    \end{array}$$ $a_k\in\mathbb{R} \Rightarrow a_k\in\mathbb{C}$
    
<div>
$$\sum_{k\in K} a_k =
    \tikz[remember picture,baseline=(akRe.base)]
	\node[minimum size=0pt,inner sep=0pt,](akRe){$ \sum_{k\in K} \Re{(a_k)} $ };
 +
    i\tikz[remember picture,baseline=(akIm.base)]
	\node[minimum size=0pt,inner sep=0pt,](akIm){$ \sum_{k\in K} \Im{(a_k)} $ };$$
</div>
...
$$\sum_{
        \begin{array}{l}
            ...
            k\in K_j\\
        \end{array}}a_{j,k}
    \text{绝对收敛于}A$$ 那么对每一 个 $j\in J$ ,
$\exists A_j \in \mathbb{C}$ .s.t. $$\sum_{k\in K_j}a_{j,k}
    \text{绝对收敛于}A_j. \;
...
有\
$$\sum_{(j,k)\in M}a_{j,k} \leqslant A$$ 而 A 是这样的最小上界.

$\forall j\in J$ 形如 $\sum_{k\in F_j}a_{j,k}$ 的每一个和都以 A 为上界.
其中 $F_j$ 是 $k_j$ 的一 个有限子集, 从而这些和式有一 个最小上界
$A_j \geqslant 0$. 且根据定义有 $\sum_{k\in K_j} =A_j$.
...
最后, 设 $A'$ 是小于$A$ 的任何一 个实数如果我们能找到一个有限集合
$G\subseteq J$ s.t $\sum_{j\in G} A_j > A'$. 证明就完成了.
...
[^4]: 当一个原有的记号被拓展包含更多种情形时，以一
    种使得一般性法则继续成立的方式来表述它的定 义，这永远是最佳选择
