...
**Remark 3**. 将 $n$ 和 $J(n)$ 以 $2$ 为基数表示 (表示为二进制). 假设:
...
注意： $0$ 移动至首位会消失, 而不需要保留空位. 
$2^{\nu(n)}-1$, 其中 $\nu(n)$ 为 $n$ 转换成的二进制数中 $1$ 的个数
...
回到第一个猜测 $J(n) = \frac{n}{2}$, $n$ 为偶数.  这个猜想在什么情况下成立？
目前已知 $n = 2^m + l$,
...
以上为求解递归式的**成套方法**(repertorire method)
