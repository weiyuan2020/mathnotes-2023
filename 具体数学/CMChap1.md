...
::: proof
*Proof.* 证明河内塔问题递归式 1. $k=0$时， $T_0 = 2^0-1 = 0$\
...
2.  求解递归式(\[\[Henei1\]](#Henei1){reference-type="ref"
    reference="Henei1"})

3.  求解递归式(\[\[HeneiRecur\]](#HeneiRecur){reference-type="ref"
    reference="HeneiRecur"})
:::

Q: 递归式$T_n= 2^n -1$是怎样得到的？ $$\begin{aligned}
    &T_0 + 1 = 1\\
    &T_n+1 = 2T_{n-1}+2 = 2(T_{n-1}+1)\end{aligned}$$ 令$U_n = T_n+1$
$$\begin{aligned}
    &U_0 = 1\\
...
平面上n条直线所界定的区域最大个数$L_n$是多少？ $$\begin{aligned}
    &L_0 = 1\\
    &L_1 = 2\\
    &L_2 = 4\\
    &L_3 = 7\\
...
$$\begin{aligned}
    ...
    &=\cdots\\
    &=L_0+1+2+\dots+n\\
    &=1+S_n\end{aligned}$$ 其中$S_n = 1+2+\dots+n$被称为**三角形数**。
$$\label{sanjiaoxingshu}
    S_n = \frac{n(n+1)}{2}$$
...
::: remark
**Remark 2**. 将直线的情况拓展到折线。 $$\begin{aligned}
        &Z_1 = 2\\
        &Z_2 = 7
    \end{aligned}$$
:::

做法，将折线补齐成两条直线[^2] $$\begin{aligned}
...
::: proof
*Proof.* 1. l is even. $$\begin{aligned}
        J(2^m+l) &= 2J(2^m+\frac{l}{2})-1\\
        &=2(2*\frac{l}{2}+1)-1\\
        &=2l+1
    \end{aligned}$$ 2. l is odd. $$\begin{aligned}
...
[]{#tab:J(n)关系式之间的关系 label="tab:J(n)关系式之间的关系"}
...
式\[\[J(n)forConst\]](#J(n)forConst){reference-type="ref"
reference="J(n)forConst"} 变为 $$\left\{
    \begin{aligned}
        A(1)&=1\\
...
当 $n = 2^m+l, 0\leqslant l< 2^m$时， 由上述三种情况可以得到 $$\left\{
    \begin{aligned}
        A(n) &       &          &= 2^m        \\
        A(n) &- B(n) &-  C(n)   &= 1          \\
        A(n) &       &+  C(n)   &= n = 2^m+l
    ...
    \begin{aligned}
        ...
        C(n) &= l
    \end{aligned}
    \right.$$ 以上为求解递归式的**成套方法**(repertorire method)
...
令$\beta_0 = \beta, \beta_1 = \gamma$ $$\left\{
    \begin{aligned}
        f(1)    &= \alpha  \\
...
[]{#tab:fnLists002 label="tab:fnLists002"}
...
Solve: 通过基数方法可知 $d = 3, c = 10$. $$\begin{aligned}
    \alpha_{1} &=  34  \\
    \alpha_{2} &=  5   \\
    \beta_{1}  &=  76  \\
    \beta_{2}  &= -2  \\
